...
double YCX(const char *pert_y, int irrep_y, double omega_y, const char *pert_c, int irrep_c, double omega_c,
    ...
    sprintf(lbl, "z_IA%s%s", pert_c, pert_x); 	
    global_dpd_->file2_init(&z1, PSIF_CC_TMP0, irrep_c^irrep_x, 0, 1, lbl);
    ...
    hyper += global_dpd_->file2_dot(&Y1, &z1);
    ...
    sprintf(lbl, "XC_IJ%s%s", pert_c, pert_x);
    global_dpd_->file2_init(&xc, PSIF_CC_TMP1, irrep_c^irrep_x, 0, 0, lbl);

    //global_dpd_->file2_scm(&xc, 0);
    ...
    // YIjAb * TIjAb //
    sprintf(lbl, "Yt_IJ%s", pert_y);
    global_dpd_->file2_init(&yt, PSIF_CC_OEI, irrep_y, 0, 0, lbl);
    ...
    sprintf(lbl, "Z(Ij,Ab)%s%s Final", pert_c, pert_x);
    global_dpd_->buf4_init(&z2, PSIF_CC_TMP0, irrep_c^irrep_x, 0, 5, 0, 5, 0, lbl);
    ...
    sprintf(lbl, "Z(Ij,Ab)%s%s", pert_c, pert_x);
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP1, irrep_c^irrep_x, 0, 5, 0, 5, 0, lbl);
    ...
    sprintf(lbl, "Z(jI,bA)%s%s", pert_c, pert_x); //Not sure
    global_dpd_->buf4_sort(&Z, PSIF_CC_TMP1, qpsr, 0, 5, lbl); //Not sure
    global_dpd_->buf4_close(&Z);
//    sprintf(lbl, "Z(jI,bA)%s%s", pert_c, pert_x);             //Not sure about this part
//    global_dpd_->buf4_init(&Z, PSIF_CC_TMP1, irrep_c ^ irrep_x, 0, 5, 0, 5, 0, lbl); //Not sure about this part
//    global_dpd_->buf4_axpy(&Z, &z2, -1); //Not sure about this part
    global_dpd_->file2_close(&X1);
