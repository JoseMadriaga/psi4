...
void Y2_homogenous_build(const char *pert, int irrep, double omega) {
    ...
    char lbl[32];
    int Gej, Gab, Gij, Ge, Gj, Gi, Ga, ij, ab, nrows, length, E, II;
    int Gbm, Gfe, bm, Gb, Gm, Gf, M, fe, ef, ncols;
    double *Y;
    dpdbuf4 S, A, B_s;
    int Gc, C, c, cc;
    ...
    double Y2_norm;
    int h, row, col, i, j, m, e, f, a, b, I_n, J;
    ...
    for (Gij = 0; Gij < moinfo.nirreps; Gij++) {
        ...
        global_dpd_->buf4_mat_irrep_init(&Z, Gij);
           for(ij = 0; ij < Z.params->rowtot[Gij]; ij++) {
               ...
               Isym = Y1.params->psym[i];
               Jsym = F.params->psym[j];
               In = i - Y1.params->poff[Isym];
               Jn = j - F.params->poff[Jsym];
               for(ab = 0; ab < Z.params->coltot[Gab]; ab++) {
                   a = Z.params->colorb[Gab][ab][0];
                   b = Z.params->colorb[Gab][ab][1];
                   Asym = Y1.params->qsym[a];
                   Bsym = F.params->qsym[b];
                   An = a - Y1.params->qoff[Asym];
                   Bn = b - F.params->qoff[Bsym];
                     if ((Isym ^ Asym) == irrep && (Jsym ^ Bsym) == 0){
                        Z.matrix[Gij][ij][ab] = Y1.matrix[Isym][In][An] * F.matrix[Jsym][Jn][Bn];
                   }
                }
           }
...
//Combine these two
    ...
    sprintf(lbl, "Z(ia|jb)%s", pert);	
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, lbl);
    
    global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 10, 10, 10, 10, 0, "WMbEj"); // I fixed here...
    ...
    global_dpd_->buf4_sort(&Z, PSIF_CC_TMP0, rpqs, 0, 5, lbl);
    global_dpd_->buf4_close(&Z);
...
//##########################TEST################################################
    ...
    sprintf(lbl, "HvvvvY2_%s (ij,ab) (%5.3f)", pert, omega);
