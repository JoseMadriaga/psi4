...
void Y2_inhomogenous_build(const char *pert, int irrep, double omega) {
    dpdfile2 Y1, X1, L1, mu1, dx, lx_ia, xl, FX, WX, z, F, GAE, GMI, t1, Y1new;  //xl -> replace this variable
    dpdbuf4 Y2, Y2new, W, WX1, L2, X2, D, LX, XD, XL, lx, lx_ijkb, Z, X1W, Z1, Z2, B;
    char lbl[32];
    int Gej, Gab, Gij, Ge, Gi, Gj, Ga, nrows, length, E, e, II;
    int Gef, Gjf, jf, fa, af, Gbm, Gfe, bm, m, Gb, Gm, Gf, M, fe, f, ef, ncols;
    double *Y;
    dpdbuf4 S, A, B_s;
    int i, j, a, b, ij, ab, Gc, C, c, cc, I_n;
...
int Isym, Jsym, Asym, Bsym, In, Jn, An, Bn;

    sprintf(lbl, "Z_%s_ijab (%5.3f)", pert, omega);
    ...
    for (Gij = 0; Gij < moinfo.nirreps; Gij++) {
        Gab = Gij ^ irrep;
        global_dpd_->buf4_mat_irrep_init(&Z, Gij);
           for(ij = 0; ij < Z.params->rowtot[Gij]; ij++) {
               i = Z.params->roworb[Gij][ij][0];
               j = Z.params->roworb[Gij][ij][1];
               Isym = L1.params->psym[i];
               Jsym = mu1.params->psym[j];
               In = i - L1.params->poff[Isym];
               Jn = j - mu1.params->poff[Jsym];
               for(ab = 0; ab < Z.params->coltot[Gab]; ab++) {
                   a = Z.params->colorb[Gab][ab][0];
                   b = Z.params->colorb[Gab][ab][1];
                   Asym = L1.params->qsym[a];
                   Bsym = mu1.params->qsym[b];
                   An = a - L1.params->qoff[Asym];
                   Bn = b - mu1.params->qoff[Bsym];
                     if ((Isym ^ Asym) == 0 && (Jsym ^ Bsym) == irrep){
                        Z.matrix[Gij][ij][ab] = L1.matrix[Isym][In][An] * mu1.matrix[Jsym][Jn][Bn];
                   }
                }
           }
    ...
    sprintf(lbl, "Z_%s_jiba (%5.3f)", pert, omega);
    global_dpd_->buf4_sort(&Z, PSIF_CC_TMP0, qprs, 0, 5, lbl);
...
/*
    ...
    global_dpd_->file2_init(&dx, PSIF_CC_OEI, 0, 0, 1, lbl);
...
*/
    ...
    sprintf(lbl, "Z_%s_ijba (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, lbl);
    global_dpd_->buf4_scm(&Z, 0.0); 

    sprintf(lbl, "DX_%s_IA (%5.3f)", pert, omega);
    global_dpd_->file2_init(&dx, PSIF_CC_OEI, irrep, 0, 1, lbl);
    global_dpd_->file2_init(&L1, PSIF_CC_LAMPS, 0, 0, 1, "LIA 0 -1");

    global_dpd_->file2_mat_init(&dx);
    global_dpd_->file2_mat_rd(&dx);
    global_dpd_->file2_mat_init(&L1);
    global_dpd_->file2_mat_rd(&L1);
    ...
    for (Gij = 0; Gij < moinfo.nirreps; Gij++) {
        ...
        global_dpd_->buf4_mat_irrep_init(&Z, Gij);
           for(ij = 0; ij < Z.params->rowtot[Gij]; ij++) {
               i = Z.params->roworb[Gij][ij][0];
               j = Z.params->roworb[Gij][ij][1];
	       Isym = dx.params->psym[i];	
	       Jsym = L1.params->psym[j];	
	       In = i - dx.params->poff[Isym];
	       Jn = j - L1.params->poff[Jsym];
	       for(ab = 0; ab < Z.params->coltot[Gab]; ab++) {	
                   a = Z.params->colorb[Gab][ab][0];
                   b = Z.params->colorb[Gab][ab][1];
            	   Asym = L1.params->qsym[a];
                   Bsym = dx.params->qsym[b];
                   An = a - L1.params->qoff[Asym];
                   Bn = b - dx.params->qoff[Bsym];       
		     if ((Isym ^ Bsym) == irrep && (Jsym ^ Asym) == 0){
		        Z.matrix[Gij][ij][ab] = dx.matrix[Isym][In][Bn] * L1.matrix[Jsym][Jn][An]; 
	       	   }
		}
           }
        global_dpd_->buf4_mat_irrep_wrt(&Z, Gij);
    ...
    global_dpd_->file2_mat_close(&dx);
...
*/
    ...
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, "Z (ji,ba)");

    sprintf(lbl, "LX_%s_jiak (%5.3f)", pert, omega); 
    global_dpd_->buf4_init(&LX, PSIF_CC_LR, irrep, 0, 11, 0, 11, 0, lbl);
    global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf");
    global_dpd_->contract444(&LX, &W, &Z, 0, 1, -1.0, 0.0); //Compute out of core!
    global_dpd_->buf4_close(&LX);
    global_dpd_->buf4_close(&W); 

    global_dpd_->buf4_sort(&Z, PSIF_CC_TMP0, qpsr, 0, 5, "Z (ij,ab)");
    global_dpd_->buf4_close(&Z);
...
*/
    ...
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "Z(ib,ja)");
    ...
    global_dpd_->buf4_sort(&Z, PSIF_CC_TMP0, prsq, 0, 5, "Z(ij,ab)");
    global_dpd_->buf4_close(&Z);
...
*/
    ...
    global_dpd_->buf4_init(&WX1, PSIF_CC_LR, irrep, 11, 11, 11, 11, 0, lbl);

    sprintf(lbl, "X_%s_IA (%5.3f)", pert, omega);
    global_dpd_->file2_init(&X1, PSIF_CC_OEI, irrep, 0, 1, lbl);
    global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf");
    global_dpd_->contract424(&W, &X1, &WX1, 3, 1, 0, 1.0, 0.0);   //compute out of core
    global_dpd_->file2_close(&X1);
    global_dpd_->buf4_close(&W);

    sprintf(lbl, "WX_%s_ibmf (%5.3f)", pert, omega);
    global_dpd_->buf4_sort(&WX1, PSIF_CC_LR, qrsp, 10, 10, lbl);
    global_dpd_->buf4_close(&WX1);
...
//    global_dpd_->buf4_close(&W);
    ...
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "Z (ia,jb)");
    ...
    global_dpd_->buf4_sort(&Z, PSIF_CC_TMP0, prqs, 0, 5, "Z(ij,ab)");
    global_dpd_->buf4_close(&Z);
...
//    global_dpd_->buf4_close(&W); 
    ...
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "Z2(ib,ja)");
    ...
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, "Z(ij,ab)");
    global_dpd_->buf4_axpy(&Z2, &Y2new, 1);
    global_dpd_->buf4_close(&Z2);
...
//    global_dpd_->buf4_close(&W);
    ...
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP1, irrep, 11, 10, 11, 10, 0, "Z2 (aj,ib)");
 
    sprintf(lbl, "LX_%s_jikb (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&LX, PSIF_CC_LR, irrep, 0, 10, 0, 10, 0, lbl);
    global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 11, 0, 11, 0, 0, "WMnIe (en,IM)"); 

    global_dpd_->contract444(&W, &LX, &Z2, 0, 1, 1, 0);
    //global_dpd_->contract444(&LX, &W, &Z2, 1, 0, 1, 0);
    global_dpd_->buf4_close(&LX);
    global_dpd_->buf4_close(&W);   

    global_dpd_->buf4_sort(&Z2, PSIF_CC_TMP1, rqps, 0, 5, "Z(ij,ab)");
    global_dpd_->buf4_close(&Z2);

    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP1, irrep, 0, 5, 0, 5, 0, "Z(ij,ab)");
    global_dpd_->buf4_axpy(&Z2, &Y2new, 1);
    global_dpd_->buf4_close(&Z2);
...
//*******************************
    ...
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, "Z2 (ji,bA)");

    global_dpd_->buf4_init(&L2, PSIF_CC_LAMPS, 0, 0, 5, 0, 5, 0, "2 LIjAb - LIjBa");
    global_dpd_->contract444(&Z, &L2, &Z2, 0, 1, 1, 0);
    global_dpd_->buf4_close(&Z);

    global_dpd_->buf4_sort(&Z2, PSIF_CC_TMP0, qpsr, 0, 5, "Z2(ij,ab)");
    global_dpd_->buf4_close(&Z2);
...
//    global_dpd_->buf4_close(&W);
    ...
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "Z (jb,ia)");
...
*/
    ...
    sprintf(lbl, "LX%s_ibja (%5.3f)", pert, omega);
...
//sort 
    ...
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "LX_ibja");
    sprintf(lbl, "X_%s_IbjA (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 10, 10, 10, 10, 0, lbl);
    global_dpd_->buf4_init(&L2, PSIF_CC_LAMPS, 0, 10, 10, 10, 10, 0, "(2 LIjAb - LIjBa) (ib|ja)");
    //Lx_ijab
...
*/
    ...
    //I think we can use the intermediate
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "LX_jbia");
    ...
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "LX_ibja");  //I am reusing
    //global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "LX_jbia");
   ...
   //sort
    global_dpd_->buf4_sort(&Z2, PSIF_CC_TMP0, rpsq, 0, 5, "Z2 (ij,ab)"); 
    global_dpd_->buf4_close(&Z2);
    ...
    //sort
    global_dpd_->buf4_sort(&Z2, PSIF_CC_TMP0, qpsr, 0, 5, "Z(ij,ab)");  //Test maybe it is worth it to sort L2?
    global_dpd_->buf4_close(&Z2);
...
*/
    ...
    */
    ...
    global_dpd_->buf4_sort(&Z2, PSIF_CC_TMP0, prsq, 0, 5, "Z2 (ij,ab)");
    //global_dpd_->buf4_sort(&Z2, PSIF_CC_TMP0, prqs, 0, 5, "Z2 (ij,ba)");
    global_dpd_->buf4_close(&Z2);
...
//    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "Z2 (ij,ba)");
//    global_dpd_->buf4_axpy(&Z2, &Y2new, 2);
//    global_dpd_->buf4_close(&Z2);


    sprintf(lbl, "XL_%s_ibme (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&Z, PSIF_CC_LR, irrep, 10, 10, 10, 10, 0, lbl);
    //global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "Xl (ib|me)");
    ...
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP1, irrep, 10, 10, 10, 10, 0, "Z(ib|ja)");
    global_dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 10, 10, 10, 10, 0, "D 2<ij|ab> - <ij|ba> (ia,jb)"); 
    global_dpd_->contract444(&Z, &D, &Z2, 0, 1, 1, 0);
    global_dpd_->buf4_close(&Z);
    global_dpd_->buf4_close(&D);

    global_dpd_->buf4_sort(&Z2, PSIF_CC_TMP1, prsq, 0, 5, "Z2 (ij,ab)");
    global_dpd_->buf4_close(&Z2);
...
//Can we emulate it above?
    ...
    sprintf(lbl, "XL_%s_ibme (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&Z, PSIF_CC_LR, irrep, 10, 10, 10, 10, 0, lbl);
    //global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "Xl (ib|me)"); //I am reusing
    global_dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 10, 10, 10, 10, 0, "D 2<ij|ab> - <ij|ba> (ia,jb)");  
    ...
    global_dpd_->buf4_sort(&Z2, PSIF_CC_TMP1, prqs, 0, 5, "Z2 (ij,ab)");
    global_dpd_->buf4_close(&Z2);

    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP1, irrep, 0, 5, 0, 5, 0, "Z2 (ij,ab)");
    global_dpd_->buf4_axpy(&Z2, &Y2new, 1);
    global_dpd_->buf4_close(&Z2);
