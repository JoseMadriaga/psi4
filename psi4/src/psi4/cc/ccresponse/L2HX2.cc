...
void L2HX2(const char *pert_x, int irrep_x, double omega_x) {
    dpdbuf4 X2, I, W, Z, Z1, Z2, W1, W2, L2;
    dpdfile2 z;
    char lbl[32];
    double Y1_norm;

    global_dpd_->buf4_init(&L2, PSIF_CC_LAMPS, 0, 0, 5, 0, 5, 0, "LIjAb");
    global_dpd_->buf4_sort(&L2, PSIF_CC_LAMPS, psqr, 10, 10, "LIbjA");
    ...
    global_dpd_->buf4_init(&L2, PSIF_CC_LAMPS, 0, 10, 10, 10, 10, 0, "LIbjA");

    global_dpd_->buf4_init(&I, PSIF_CC_DINTS, 0, 10, 10, 10, 10, 0, "D <ij|ab> (ib,ja)");
    global_dpd_->contract444(&I, &L2, &Z, 0, 0, 1, 0);
    ...
    //sprintf(lbl, "X_%s_(2IAjb-jAIb) (%5.3f)", pert_x, omega_x);
    //global_dpd_->buf4_init(&Y2, PSIF_CC_LR, irrep_x, 10, 10, 10, 10, 0, lbl);
    ...
    global_dpd_->contract444(&I, &L2, &Z, 0, 1, 0.5, 0);
    global_dpd_->buf4_close(&I);
    global_dpd_->buf4_close(&L2);
    ...
    global_dpd_->contract444(&Z1, &L2, &Z, 1, 1, 1, 1);
    global_dpd_->buf4_close(&L2);
    global_dpd_->buf4_close(&Z1);
    global_dpd_->buf4_close(&Z);
    ...
    //global_dpd_->buf4_init(&Y2, PSIF_CC_LR, irrep_x, 0, 5, 0, 5, 0, lbl);
    global_dpd_->buf4_init(&L2, PSIF_CC_LAMPS, 0, 0, 5, 0, 5, 0, "LIjAb");
    global_dpd_->contract442(&L2, &I, &z, 3, 3, -1, 0);
    global_dpd_->buf4_close(&L2);
    global_dpd_->buf4_close(&I);
    ...
    global_dpd_->buf4_init(&L2, PSIF_CC_LAMPS, 0, 0, 5, 0, 5, 0, "LIjAb");
     //sprintf(lbl, "X_%s_IjAb (%5.3f)", pert_x, omega_x);
    //global_dpd_->buf4_init(&Y2, PSIF_CC_LR, irrep_x, 0, 5, 0, 5, 0, lbl);
    global_dpd_->contract424(&L2, &z, &Z1, 3, 1, 0, 1, 0);
    global_dpd_->buf4_close(&L2);
    global_dpd_->file2_close(&z);
    global_dpd_->buf4_axpy(&Z1, &Z, 1);
    global_dpd_->buf4_sort(&Z1, PSIF_CC_TMP1, qpsr, 0, 5, "Z(jI,bA)");
    ...
    //sprintf(lbl, "X_%s_IjAb (%5.3f)", pert_x, omega_x);
    //global_dpd_->buf4_init(&Y2, PSIF_CC_LR, irrep_x, 0, 5, 0, 5, 0, lbl);
    global_dpd_->contract442(&I, &L2, &z, 0, 0, 1, 0);
    global_dpd_->buf4_close(&L2);
    global_dpd_->buf4_close(&I);
    ...
    //sprintf(lbl, "X_%s_IjAb (%5.3f)", pert_x, omega_x);
    //global_dpd_->buf4_init(&Y2, PSIF_CC_LR, irrep_x, 0, 5, 0, 5, 0, lbl);
    global_dpd_->contract244(&z, &L2, &Z1, 0, 0, 0, 1, 0);
    global_dpd_->buf4_close(&L2);
    global_dpd_->file2_close(&z);
    global_dpd_->buf4_axpy(&Z1, &Z, -1);
    global_dpd_->buf4_sort(&Z1, PSIF_CC_TMP1, qpsr, 0, 5, "Z(jI,bA)");
