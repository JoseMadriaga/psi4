...
void Y1_inhomogenous_build(const char *pert, int irrep, double omega) {
    ...
    dpdfile2 F, z;
    dpdfile2 Y1, Y1new, X1, L1, mu1, GAE, GMI, dx, lt, lx, lx_ia;
    dpdbuf4 W, WL, D, X2, Z2, Z3, LX_iajb; 
    dpdbuf4 L2, Z, mu2, XW_ijab, LX_ijab;
    char lbl[32];
    ...
    global_dpd_->contract422(&D, &X1, &dx, 0, 0, 1, 1);
    global_dpd_->file2_axpy(&dx, &Y1new, 2, 0);
    ...
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "Z (ij,ab)");   //Revisit it!
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "Z (ji,ba)");  //Revisit it!
...
//    global_dpd_->file2_close(&GMI);
    ...
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "GAED (ij,ab)");
    ...
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "GMID (ij,ab)");
    ...
    global_dpd_->file2_init(&z, PSIF_CC_TMP0, irrep, 0, 1, lbl);
    sprintf(lbl, "X_%s_IjAb (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl);
    global_dpd_->file2_init(&L1, PSIF_CC_LAMPS, 0, 0, 1, "LIA 0 -1"); 
    global_dpd_->dot24(&L1, &X2, &z, 0, 0, 2, 0); 
    
    sprintf(lbl, "X_%s_IjAb (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl);  
    global_dpd_->dot23(&L1, &X2, &z, 0, 0, -1, 1);
    global_dpd_->buf4_close(&X2);
 
    global_dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D 2<ij|ab> - <ij|ba>");
    global_dpd_->dot24(&z, &D, &Y1new, 0, 0, 1, 1); 
    global_dpd_->buf4_close(&D); 
    ...
    global_dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl);
    global_dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D 2<ij|ab> - <ij|ba>");
    global_dpd_->contract442(&X2, &D, &z, 0, 0, 1.0, 0.0);
    global_dpd_->contract222(&z, &L1, &Y1new, 1, 1, -1.0, 1.0);
    global_dpd_->file2_close(&z);

    sprintf(lbl, "Z_%s_AE (%5.3f)", pert, omega);
    global_dpd_->file2_init(&z, PSIF_CC_OEI, irrep, 1, 1, lbl);

    sprintf(lbl, "X_%s_IjAb (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl);
    global_dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D 2<ij|ab> - <ij|ba>");
    global_dpd_->contract442(&X2, &D, &z, 2, 2, -1.0, 0.0);
  
    global_dpd_->buf4_close(&X2);
    global_dpd_->buf4_close(&D);
    global_dpd_->contract222(&L1, &z, &Y1new, 0, 1, 1.0, 1.0);
    global_dpd_->file2_close(&z);
    global_dpd_->file2_close(&L1);
...
//Fix it!
    ...
    global_dpd_->buf4_init(&LX_iajb, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "LXiajb");
    ...
    global_dpd_->buf4_sort(&W, PSIF_CC_TMP0, spqr, 5, 10, "WAmEf (AE,mf)");  //sort  
    global_dpd_->buf4_close(&W);

    global_dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 5, 10, 5, 10, 0, "WAmEf (AE,mf)");

    global_dpd_->contract442(&LX_iajb, &W, &Y1new, 0, 0, -1, 1);
...
*/
    ...
    sprintf(lbl, "LX_%s_iajb (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&LX_iajb, PSIF_CC_LR, irrep, 10, 10, 10, 10, 0, lbl);

    sprintf(lbl, "X_%s_IbjA (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 10, 10, 10, 10, 0, lbl);
    global_dpd_->buf4_init(&L2, PSIF_CC_LAMPS, 0, 10, 10, 10, 10, 0, "(2 LIjAb - LIjBa) (ia|jb)");
    global_dpd_->contract444(&L2, &X2, &LX_iajb, 1, 1, 1, 0);
    global_dpd_->buf4_close(&L2);
    global_dpd_->buf4_close(&X2);

    sprintf(lbl, "LX_%s_ajib (%5.3f)", pert, omega);
    global_dpd_->buf4_sort(&LX_iajb, PSIF_CC_TMP0, qrps, 11, 10, lbl);       
    global_dpd_->buf4_close(&LX_iajb);

    sprintf(lbl, "LX_%s_ajib (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&LX_iajb, PSIF_CC_TMP0, irrep, 11, 10, 11, 10, 0, lbl);

    global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf"); //Compute this part out of core
    global_dpd_->contract442(&LX_iajb, &W, &Y1new, 2, 2, -1, 1);
    global_dpd_->buf4_close(&LX_iajb);
    global_dpd_->buf4_close(&W);
...
/*
    ...
    global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf");
    global_dpd_->contract442(&W, &LX_iajb, &Y1new, 1, 1, -1, 1);
...
*/
    ...
    // X2 x  Hvovv
    sprintf(lbl, "X2WAmEf_%s_ijab (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&XW_ijab, PSIF_CC_LR, irrep, 0, 11, 0, 11, 0, lbl);

    sprintf(lbl, "X_%s_IjAb (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl);
    global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf"); //Compute this part out of core
    global_dpd_->contract444(&X2, &W, &XW_ijab, 0, 0, 1, 0);
    global_dpd_->buf4_close(&X2);
    global_dpd_->buf4_close(&W);

    global_dpd_->buf4_init(&L2, PSIF_CC_LAMPS, 0, 0, 5, 0, 5, 0, "2 LIjAb - LIjBa");
    global_dpd_->contract442(&XW_ijab, &L2, &Y1new, 3, 3, -1, 1);
    global_dpd_->buf4_close(&XW_ijab);
    global_dpd_->buf4_close(&L2);
...
//FIX IT!!!
    ...
    global_dpd_->buf4_init(&LX_iajb, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "LX (ia,jb)");
    ...
    global_dpd_->buf4_sort(&LX_iajb, PSIF_CC_TMP0, qrps, 11, 10, "LX (ij|ab)");
    //global_dpd_->buf4_sort(&LX_iajb, PSIF_CC_TMP1, qrsp, 11, 11, "LX (ij|ab)");
    global_dpd_->buf4_close(&LX_iajb);

    global_dpd_->buf4_init(&LX_iajb, PSIF_CC_TMP0, irrep, 11, 10, 11, 10, 0, "LX (ij|ab)");
    //global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf 2(Am,Ef) - (Am,fE) (Em,Af) test");   
...
/*
    global_dpd_->buf4_sort(&LX_iajb, PSIF_CC_LR, qrsp, 11, 11, "LX (aj|ib)");
    global_dpd_->buf4_close(&LX_iajb);

    global_dpd_->buf4_init(&LX_iajb, PSIF_CC_LR, irrep, 11, 11, 11, 11, 0, "LX (aj|ib)");
    //global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf 2(Am,Ef) - (Am,fE)");
    global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf 2(Am,Ef) - (Am,fE) (am,fe)");	
    global_dpd_->contract442(&LX_iajb, &W, &Y1new, 3, 3, 1, 1);
    global_dpd_->buf4_close(&LX_iajb);
    global_dpd_->buf4_close(&W);
*/
...
//        global_dpd_->file2_init(&lx, PSIF_CC_OEI, irrep, 1, 1, "Lx_AB");
    ...
    global_dpd_->buf4_init(&LX_ijab, PSIF_CC_TMP0, irrep, 0, 0, 0, 0, 0, lbl);
...
//MK I am here!!!
    ...
    sprintf(lbl, "LX_%s_IAJB (%5.3f)", pert, omega);
    global_dpd_->buf4_init(&LX_iajb, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, lbl);
    ...
    sprintf(lbl, "LX_%s_AIJB (%5.3f)", pert, omega);
    global_dpd_->buf4_sort(&LX_iajb, PSIF_CC_TMP0, qrps, 11, 10, lbl);
    ...
    global_dpd_->contract442(&W, &LX_iajb, &Y1new, 0, 0, 1, 1);
    global_dpd_->buf4_close(&LX_iajb);
    global_dpd_->buf4_close(&W);
    ...
    global_dpd_->buf4_init(&LX_iajb, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "LX_iajb");  
    ...
    global_dpd_->buf4_sort(&LX_iajb, PSIF_CC_TMP0, rqps, 10, 10, "LX_jaib");
    global_dpd_->buf4_close(&LX_iajb);

    global_dpd_->buf4_init(&LX_iajb, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, "LX_jaib");
    global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 0, 10, 0, 10, 0, "WMnIe"); //Fixed here
    ...
    global_dpd_->file2_init(&lx, PSIF_CC_OEI, irrep, 0, 0, lbl);
    //global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "2WMnIe - WnMIe (Mn,eI)");
    global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "2WMnIe - WnMIe (In,eM)");
    global_dpd_->dot14(&lx, &W, &Y1new, 0, 0, -1.0, 1.0);
    //global_dpd_->dot24(&lx, &W, &Y1new, 0, 0, -1.0, 1.0);
    global_dpd_->file2_close(&lx);
    global_dpd_->buf4_close(&W);

/*
    sprintf(lbl, "Lx_%s_IJ_c (%5.3f)", pert, omega);
    global_dpd_->file2_init(&lx, PSIF_CC_OEI, irrep, 0, 0, lbl);
    ...
    global_dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl);
    global_dpd_->buf4_init(&L2, PSIF_CC_LAMPS, 0, 0, 5, 0, 5, 0, "2 LIjAb - LIjBa");
    global_dpd_->contract442(&X2, &L2, &lx, 0, 0, 1.0, 0.0);
    global_dpd_->buf4_close(&X2);
    global_dpd_->buf4_close(&L2);
...
outfile->Printf("\t\n I am TEST12a");
    ...
    global_dpd_->dot14(&lx, &W, &Y1new, 0, 0, -1.0, 1.0);         //CHECK THIS!!!!!!!!! 
...
*/
    ...
    global_dpd_->buf4_sort(&LX_iajb, PSIF_CC_TMP0, qrsp, 11, 11, "LX_ajbi");
    global_dpd_->buf4_close(&LX_iajb);

    global_dpd_->buf4_init(&LX_iajb, PSIF_CC_TMP0, irrep, 11, 11, 11, 11, 0, "LX_ajbi");
    global_dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "2WMnIe - WnMIe (Mn,eI)");
    global_dpd_->contract442(&W, &LX_iajb, &Y1new, 0, 0, -1, 1);   
    global_dpd_->buf4_close(&LX_iajb);
    global_dpd_->buf4_close(&W);
