...
TwoBodyAOInt* IntegralFactory::f12_double_commutator_libint2(std::vector<std::pair<double,double>> ctg_params, int deriv, bool use_shell_pairs) {
     auto integral_package = Process::environment.options.get_str("INTEGRAL_PACKAGE");
     auto threshold = Process::environment.options.get_double("INTS_TOLERANCE");
     if (integral_package == "LIBINT2") return new Libint2F12DoubleCommutator(ctg_params, this, threshold, deriv, use_shell_pairs);
     else throw PSIEXCEPTION("Libint2F12DoubleCommutator ERIs with ctg_params are only available in the Libint2 Package");
}
